<html>
<head>
<title>Delete Active Client</title>
</head>
<body>
<style>
...
//output current data in active_clients

$active_client = $conn->query("SELECT * FROM active_clients") or die($conn->error);
while ($row = $active_client->fetch_assoc())
{
    echo "<tr>";
    echo "<td>".$row['name']."</td>";
...
if (isset($_POST['t1']))
{
    ...
    if ($name)
    {
        $result = $conn->query("DELETE FROM active_clients WHERE name = '$name'");
        // echo "Deletion complete. '$name''s information can still be found in Client.";
    }
    else
        echo "Deletion failed. <br>".$conn->error;
}
$conn->close();
?>